...
function getExportFileName(original: string, format: ExportFormat, groupNumber?: number): string {
  const base = original.replace(/\.[^.]+$/, '')
  const prefix = groupNumber != null ? `G${groupNumber}_` : ''
  return `${prefix}${base}-blurred.${format === 'jpeg' ? 'jpg' : 'png'}`
}
...
export async function exportImage(
  ...
  const fileName = getExportFileName(image.fileName, format, groupNumber)
  ...
  if (isMobileDevice() && navigator.share && navigator.canShare) {
    ...
    if (navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: shareTitle ?? 'Blurred photo' })
        return
      } catch (e) {
